...
module.exports = function ({ accountCredentials, backupDirectory, prettyPrintJSON, specifiedCollection }) {
	return Promise.resolve()
		...
		.then(() => {
			...
			return Firebase.firestore().getCollections().then((collections) =>
				eachPromise.serial(
					collections.filter(collection => collection.id === specifiedCollection || !specifiedCollection)
                    .map((collection) =>
						() => backupCollection({
							collection,
							prettyPrintJSON,
